...
describe('sumOfBuyCost', () => {
    ...
    test('sum all the cost for the BUY trades', () => {
        expect(buySummary(trades).cost).toEqual(27675)
        expect(buySummary(trades).units).toEqual(5300)
        expect(buySummary(trades).fees).toEqual(179.55)
    })
    test('sum all the cost for the SELL trades', () => {
        expect(sellSummary(trades).cost).toEqual(7525)
        expect(sellSummary(trades).units).toEqual(2050)
        expect(sellSummary(trades).fees).toEqual(59.85)
    })
})
...
describe('calculateBuyAverages', () => {
    ...
    test('with only one transaction', () => {
        const { average, outstandingUnits } = calculateBuyAverages(trades.slice(0,1))
        expect(average).toEqual(57)
        expect(outstandingUnits).toEqual(100)
    })
    test('return average and outstanding numbers', () => {
        const { average, outstandingUnits } = calculateBuyAverages(trades.slice(0,2))
        expect(average).toEqual(57.5)
        expect(outstandingUnits).toEqual(200)
    })
    test('calculateSellAverages', () => {
        const { average, outstandingUnits } = calculateSellAverages(trades.slice(0,2), trades[2])
        expect(average).toEqual(57.5)
...
describe('averagePriceForEachTransaction',  ()=> {
    ...
    test('buy/buy/buy/sell', () => {
        const result = averagePriceForEachTransaction(trades)
