...
export default function Transctions(props) {
    ...
    function calcTotal() {
        const totalObj = list.reduce((acc, transaction) => {
            return {
                debit: transaction.amount < 0 ? parseFloat(transaction.amount) + acc.debit : acc.debit,
                credit: transaction.amount >= 0 ? parseFloat(transaction.amount) + acc.credit : acc.credit,
            ...
            <Table>
                <thead>
                    <tr>
                        ...
                        <th onClick={() => sort('amount')}>
                            Amount
                        </th>
                        <th onClick={() => sort('account')}>
                            Account
                        </th>
